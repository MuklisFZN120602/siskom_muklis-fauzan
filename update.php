  ...
  // mengambil data dari form yang memiliki button submit bernama update
    if(isset($_POST['update'])){
        ...
        if(move_uploaded_file($tempname, $folder)){
            $sql = "UPDATE pantai SET name='$nama_pantai', lokasi='$lokasi', review='$review', harga_tiket=$harga_tiket, jam_buka='$jam_buka', gambar='$gambar' WHERE id=$id_to_update";
        } else {
            $sql = "UPDATE pantai SET name='$nama_pantai', lokasi='$lokasi', review='$review', harga_tiket=$harga_tiket, jam_buka='$jam_buka', gambar='$gambar' WHERE id=$id_to_update";
        }
        $result = $conn->query($sql);
